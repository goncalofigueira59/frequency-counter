...
struct VARIAVEIS_TENSAO {
  ...
  int valorinstantanea[625];    // Array para armazenar leituras da tensão
  int maiorvalor;              //Maior valor de tensao medida pelo o arduino
...
void calcularTensao() {
  ...
  for (int i = 0; i < 625; i++) {
    ...
    tensao.valorinstantanea[i] = analogRead(pinosensor);  //Faz leituras da tensão
  ...
  for (int i = 0; i < 625; i++) {
    ...
    tensao.maiorvalor = max(tensao.maiorvalor, tensao.valorinstantanea[i]);  // Identifica o maior valor lido
  }
}

tensao.tensaopico = ((tensao.maiorvalor * 10) / 1024) * 1.11;   //Calcula a tensão de pico
tensao.tensaorms = tensao.tensaopico / 1.4;                    //Calcula a tensão RMS
